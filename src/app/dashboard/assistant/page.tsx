...
function VerificationGatekeeper({
  ...
  onRedirect: () => void;
    ...
    const messages = {
        verifying: { title: "Verifying Your Setup", text: "Please wait while we check your campus configuration..." },
        needs_setup: { title: "Setup Required", text: "Welcome! You must set up your campus before you can access the dashboard." },
        error: { title: "Verification Failed", text: "We couldn't verify your account status. Please try again later." }
    };
    ...
    return (
        ...
        >
            <div className="bg-orange-100 p-8 rounded-lg shadow-xl max-w-md border border-gray-700">
                <h2 className={`text-2xl font-bold text-black mb-4 ${tektur.className}`}>{currentMessage.title}</h2>
                <p className="text-gray-700 mb-6">{currentMessage.text}</p>
                
                {status === 'verifying' && <div className="w-8 h-8 border-4 border-dashed rounded-full animate-spin border-indigo-400 mx-auto"></div>}
                ...
                {status === 'needs_setup' && (
                 ...
                 {status === 'error' && (
                    <p className="text-red-400 text-sm">Please refresh the page or contact support.</p>
                )}
            </div>
        </motion.div>
    );
...
const DashboardContent = () => {
  ...
  const stats = [
    { label: "Total Students", value: summary.totalStudents, color: "text-cyan-300" },
    { label: "Total Teachers", value: summary.totalTeachers, color: "text-green-300" },
    { label: "Attendance Today", value: summary.totalAttendance, color: "text-yellow-300" },
    { label: "Tokens Today", value: summary.tokensToday, color: "text-purple-300" },
  ];

  return (
    <div className={`${styles.scrollbarHide} text-white`}>
      <div className={`grid grid-cols-1 md:grid-cols-4 px-4 gap-4 mb-6 mt-16 ${tektur.className}`}>
...
export default function AdminDashboardPage() {
  ...
  const [verificationStatus, setVerificationStatus] = useState<'verifying' | 'needs_setup' | 'complete' | 'error'>('verifying');

  useEffect(() => {
    const assistantID = localStorage.getItem('assistantId');
    const checkAssistantSetup = async () => {
      try {
        const response = await fetch(`/api/assistant/details?assistantId=${assistantID}`);
        const data = await response.json();

        if (!response.ok) {
          if (response.status === 401) {
            // If unauthorized, the session is invalid, so redirect to login
        ...
        if (!data.campusId) {
          // If the admin has no campus, they need to go to the setup page.
          setVerificationStatus('needs_setup');
        } else {
          // If they have a campus, the setup is complete.
  ...
  return (
    <>
      {/* The dashboard content is always rendered in the background, but will be covered by the modal if needed. */}
      <DashboardContent />

      {/* The "gatekeeper" modal is rendered on top. It will smoothly disappear when verification is complete. */}
      <AnimatePresence>
