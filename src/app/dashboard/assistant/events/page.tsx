...
const Page = () => {
  ...
  const handleDelete = async () => {
    ...
    // Assumes 'campusId' is available in your component's state or props
    if (!campusId) {
      showErrorMessage("Campus could not be identified. Please refresh.");
      return;
    }

    const toastId = showLoadingMessage("Deleting event...");

    try {
      const res = await fetch(`/api/assistant/event/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        // ADD the campusId to the request body
        body: JSON.stringify({
      ...
      fetchEvents(campusId); // Refresh the event list
    } catch (err: any) {
