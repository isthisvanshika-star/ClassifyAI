...
import { Sparkles, TrendingUp, AlertTriangle, FileText, Loader2, BookOpen } from "lucide-react";

const fetcher = (url: string) => fetch(url).then((res) => res.json());

export default function ExamPredictor({ subjectId, subjectName }: { subjectId: string, subjectName: string }) {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-slate-900/50 rounded-3xl border border-white/5">
        ...
        <h3 className="text-xl font-bold text-white mb-2">Analyzing Past Papers...</h3>
        <p className="text-gray-400 text-sm">Classify AI is finding patterns in {subjectName} PYQs.</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 bg-slate-900/50 rounded-3xl border border-red-500/20">
        ...
        <p className="text-red-300 font-medium text-center">
          {data?.error || "Failed to generate predictions. Tell your teacher to upload more PYQs!"}
        </p>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-4 mb-8 relative z-10">
        ...
        <div>
          ...
          <p className="text-sm text-cyan-400 font-semibold flex items-center gap-2 mt-1">
            <TrendingUp size={16} /> Based on {data.totalPapersAnalyzed} past papers of {subjectName}
          </p>
        </div>
      </div>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider
                    ${item.weightage === 'High' ? 'bg-red-500/20 text-red-400 border border-red-500/20' : 
                      item.weightage === 'Medium' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/20' : 
                ...
                <div className="relative flex items-center justify-center w-16 h-16">
                  <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle 
                      ...
                      strokeDashoffset={(28 * 2 * Math.PI) - ((item.probability / 100) * (28 * 2 * Math.PI))}
                      className="text-cyan-400 transition-all duration-1000 ease-out" 
                      strokeLinecap="round"
                    />
                  </svg>
                  <div className="absolute flex flex-col items-center">
                    <span className="text-sm font-bold text-white">{item.probability}%</span>
                  </div>
                </div>
          ...
          >
            ...
            >
              <div className="text-center py-10">
                ...
                <h3 className="text-2xl font-bold text-white mb-2">Generating Answer...</h3>
                <p className="text-gray-400">Classify AI is scanning uploaded notes for "{selectedTopic}".</p>
                <p className="text-xs text-violet-400 mt-4">(Auto-solution feature coming next!)</p>
