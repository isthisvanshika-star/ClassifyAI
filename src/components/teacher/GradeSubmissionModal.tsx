...
export default function GradeSubmissionModal({
  ...
  onClose: () => void;
  onSuccess: () => void;
  submission: any; // The full submission object
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onSuccess(); // Re-fetches data on the parent page
          ...
          <div className="space-y-4">
            {submission.fileUrl && (
              <Link href={submission.fileUrl} target="_blank" rel="noopener noreferrer">
                <div className="block w-full text-center bg-gray-700 hover:bg-gray-600 p-3 rounded-md text-indigo-300 font-semibold transition-colors">
                  View Submitted File
                </div>
              </Link>
            )}
            {submission.text && (
                <div className="bg-gray-900/50 p-3 rounded-md border border-gray-600">
                    <p className="text-sm font-semibold text-gray-400 mb-1">Text Submission:</p>
