...
export default function AssignmentHeader({ assignment, handleStatusChange, isStatusLoading }: AssignmentHeaderProps) {
  return (
    <motion.header
      ...
      className="mb-10"
    >
      <Link href="/dashboard/teacher/assignments" className="text-sm text-cyan-400 hover:underline mb-4 inline-block">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex flex-col items-end gap-2">
          {assignment.status === "DRAFT" ? (
            <motion.div
              onClick={() => !isStatusLoading && handleStatusChange("PUBLISHED")}
          ...
          {assignment.status === "PUBLISHED" && assignment.submissions.length > 0 && (
            <p className="text-[10px] text-gray-500 italic leading-tight">Cannot revert: Submissions exist.</p>
          )}
        </div>
      </div>

      <div className="flex flex-wrap gap-4 mt-3 text-gray-400 text-sm">
        <span className="px-3 py-1 rounded-full bg-white/10">{assignment.subject.name}</span>
        <span className="px-3 py-1 rounded-full bg-white/10">
          Due: {assignment.dueDate ? new Date(assignment.dueDate).toLocaleString() : "No due date"}
        </span>
        <span className="px-3 py-1 rounded-full bg-white/10">Max Marks: {assignment.totalMarks || "N/A"}</span>
      </div>
    </motion.header>
  );
}